...
@Singleton
class EoriServiceConnector @Inject() (httpClientV2: HttpClientV2, appConfig: EoriServiceConnector.Config)(implicit ec: ExecutionContext)
  ...
  def postMessage(soapRequest: NodeSeq, headers: Seq[(String, String)])(implicit hc: HeaderCarrier): Future[SendResult] = {
    postHttpRequest(soapRequest, headers, s"${appConfig.baseUrl}/${appConfig.path}").map {
      ...
      .recoverWith {
        case NonFatal(e) =>
          logger.warn(s"NonFatal error ${e.getMessage} while forwarding message in EoriServiceConnector.postMessage", e)
