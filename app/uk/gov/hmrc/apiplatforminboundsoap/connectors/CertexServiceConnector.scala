...
@Singleton
class CertexServiceConnector @Inject() (httpClientV2: HttpClientV2, appConfig: CertexServiceConnector.Config)(implicit ec: ExecutionContext)
  ...
  def postMessage(soapRequest: NodeSeq, headers: Seq[(String, String)])(implicit hc: HeaderCarrier): Future[SendResult] = {
    postHttpRequest(soapRequest, headers, s"${appConfig.baseUrl}/${appConfig.path}").map {
      case Right(response)                                        => SendSuccess(response.status, response.body)
      ...
      .recoverWith {
        case NonFatal(e) =>
          logger.warn(s"NonFatal error ${e.getMessage} while forwarding message in CertexServiceConnector.postMessage", e)
